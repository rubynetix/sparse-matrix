...
module MatrixIdentitiesTestCase
  ...
  def tst_general_matrix_identities
    ...
    assert_equal(@a_inv + @b_inv, @a_inv * (@a + @b) / @b)
    assert_equal(@i + @a_inv, (@i + @a) / @a)
    assert_equal((@i + (@a_inv * @b)).inverse, (@a + @b).inverse * @a)
    assert_equal((@i + (@a * @b)).inverse, (@a_inv + @b).inverse / @a)
    assert_equal(@a + @b_inv, @a * (@a_inv + @b) / @b)
    assert_equal(@a - @b, (@a * -1) * (@a_inv - @b_inv) * @b)
    assert_equal((@a * @b) + (@b * @c), @a * ((@a_inv * @b) + (@b / @c)) * @c)
    assert_equal(@a + (@a * @b), (@a + (@a / @b)) * @b)
    assert_equal((@a + @b).inverse, @a_inv / (@a_inv + @b_inv) / @b)
    assert_equal((@a_inv + @b_inv).inverse, @a / (@a + @b) * @b)
    assert_equal((@i + @a_inv).inverse, (@i + @a).inverse * @a)
    assert_equal((@a + @b).inverse, @a_inv / (@a_inv + @b_inv) / @b)
    assert_equal((@i + @a).inverse, @a / (@i + @a) / @a)
    assert_equal((@i - @a).inverse, @a / (@i - @a) / @a)
    assert_equal(@b / (@i + (@a * @b)), @a_inv / (@i + (@a * @b)) * (@a * @b))
    assert_equal((@i + (@a * @b)).inverse * @a, @a * @b / (@i + (@a * @b)) * (@a * @b))
    assert_equal(@a - (@a * (@b_inv * @a)), @a * (@a_inv - @b_inv) * @a)
    assert_equal(@a * (@a_inv + @b_inv), (@a + @b) / @b)
    assert_equal(@a * (@a_inv + @b), (@a + @b_inv) * @b)
    assert_equal(@a / (@a + @b), (@a_inv + @b_inv).inverse / @b)
    assert_equal(@a_inv * (@a + @b), @a * (@a_inv + @b_inv))
    assert_equal((@a / @b) + @i, @a * (@a_inv + @b_inv))
